...
def do_stats(tfr, name, contrast):
    ...
    np.save(f"data/parcel_analysis/group_level/{name}_contrast_{contrast}_pvalues.npy", pvalues)

os.makedirs("data/parcel_analysis/group_level", exist_ok=True)
for contrast in range(4):

    # Total power response
    total = np.array([np.load(file) for file in sorted(glob(f"data/parcel_analysis/first_level/total_*_contrast_{contrast}.npy"))])
    do_stats(total, "total", contrast)

    # Evoked response
    evoked = np.array([np.load(file) for file in sorted(glob(f"data/parcel_analysis/first_level/evoked_*_contrast_{contrast}.npy"))])
