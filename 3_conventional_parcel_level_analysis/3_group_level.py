...
for contrast in range(4):
    ...
    first_level_files = sorted(glob(f"data/parcel_analysis/first_level/total_*_contrast_{contrast}.npy"))
    total = np.array([np.load(file) for file in first_level_files])
    do_stats(total, "total", contrast)

    # Evoked response
    first_level_files = sorted(glob(f"data/parcel_analysis/first_level/evoked_*_contrast_{contrast}.npy"))
