...
t = np.load(f"data/hmm/run{run:02d}/first_level/t.npy")

os.makedirs(f"data/hmm/run{run:02d}/group_level", exist_ok=True)
for contrast in range(4):

    # Load epoched state time courses for each subject and run
    first_level_files = sorted(glob(f"data/hmm/run{run:02d}/first_level/*_contrast_{contrast}.npy"))
    ...
    # Save
    np.save(f"data/hmm/run{run:02d}/group_level/contrast_{contrast}.npy", epochs)
    np.save(f"data/hmm/run{run:02d}/group_level/contrast_{contrast}_pvalues.npy", pvalues)
